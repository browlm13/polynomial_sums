...
# testing on recurivie formula with a negitive q/p
degree = 3
h = -1
x0 = 0
y0 = 300
q = np.array([23,5,10,-2])
...
N = 20
x_values = np.linspace(-N/2,N/2,N*10) 
y_values = p(x_values)# computing the values of sin(x)/x


import pandas as pd 
df = pd.DataFrame(list(zip(x_values, y_values)), columns=['x', 'y'])
df = df.set_index('x')
print(df)
...
ax = seaborn.lineplot(data=df, color="coral", label="line")
plt.show()
